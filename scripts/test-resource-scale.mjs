...
try {
  const service = new ResourceKnowledgeService(root, new ContentStore(join(root, 'content')))
  ...
  const canceledRoot = join(root, 'cancelled'); const canceledService = new ResourceKnowledgeService(canceledRoot, new ContentStore(join(canceledRoot, 'content'))); const controller = new AbortController(); let pages = 0
