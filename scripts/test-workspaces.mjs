...
try {
  ...
  const before = await app.evaluate('window.monitor.getWorkspaces()')
  ...
  const listedByAgent = await controlRequest('/workspaces')
  ...
  const created = await app.evaluate("window.monitor.createWorkspace({ name: 'Alice', profile: 'H' })")
  ...
  const opened = await app.evaluate(`window.monitor.openWorkspace(${JSON.stringify(created.id)})`)
  await app.waitConnected(1)
  ...
  const third = await controlRequest('/workspaces', {
    method: 'POST',
  ...
  const suspended = await controlRequest(`/workspaces/${encodeURIComponent(created.id)}/suspend`, { method: 'POST' })
  ...
  const resumed = await app.evaluate("window.monitor.openWorkspace('default')")
  await app.waitConnected(1)
